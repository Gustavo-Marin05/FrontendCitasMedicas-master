    ...
    .then(([patients, records]) => {
      ...
      records.forEach((record) => {
        ...
        row.innerHTML = `
          ...
          <td>
            <button class="edit-button" onclick='editRecord(${JSON.stringify(
              record
            )})'>Editar</button>
            <button class="delete-button" onclick='deleteRecord(${
              record.id
            })'>Eliminar</button>
          </td>
        `;
...
function editRecord(record) {
  ...
  document.getElementById("created_at").value = record.created_at;
  document.getElementById("recordForm").style.display = "block";
